    ...
    <section ref={ref} className="py-24 bg-white">
      <div className="max-w-350 mx-auto px-6">
        <div className="mb-10 reveal">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 font-sans tracking-tight">Rent Luxury Properties</h1>
          <p className="text-gray-500 mt-2">Find premium rentals in top communities.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[1,2,3].map((i) => (
            <motion.div key={i} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} className="border border-gray-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all reveal">
              ...
              <div className="p-6">
                <h3 className="text-lg font-bold text-gray-900">Luxury Apartment {i}</h3>
                <p className="text-gray-500 text-sm mt-1">3BHK, furnished, city view.</p>
              </div>
            </motion.div>
          ))}
        </div>
